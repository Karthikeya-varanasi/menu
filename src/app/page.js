...
export default function MenuPage() {
  ...
  const saveOrder = async () => {
    const order = { items: cart, subTotal, cgst, sgst, total };
        ...
        <div className="mt-5">
          ...
          {menuSections.map((section, sIndex) => (
            <div key={sIndex} className="mb-4">
              {section.title && <h5 className="mb-3">{section.title}</h5>}
          ...
          {cart.length >= 1 && (
            <button className="btn btn-success w-100 mt-4" onClick={() => setShowBill(true)}>Check Bill</button>
          )}
        </div>
      )}
      ...
      {showBill && !showFinalPrint && (
        <>
          <div className="text-end mb-3 d-flex justify-content-end gap-2">
            <button className="btn btn-warning btn-sm" onClick={() => setShowBill(false)}>Add More Items</button>
            <button className="btn btn-danger btn-sm" onClick={() => window.location.href = '/'}>Cancel Order</button>
          </div>
          ...
          <div className="mt-5">
            ...
            <button className="btn btn-dark w-100 mt-3" onClick={saveOrder}>Submit Order</button>
          </div>
        </>
      )}
